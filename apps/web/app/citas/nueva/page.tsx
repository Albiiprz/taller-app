...
function NuevaCitaForm() {
  ...
  async function loadAvailability() {
    ...
    setAfternoonSlots([]);
    try {
      const rows = await getTechniciansAvailabilityByDate({ date, durationMinutes: duration });
      setTechnicians(rows);
  ...
  useEffect(() => { void loadAvailability(); }, [date, duration]);
  ...
  async function saveAppointment() {
    ...
    try {
      const res = await createAppointment({
        client: { name: name.trim(), phone: phone.trim(), email: email.trim() || undefined, company: company.trim() || undefined },
        vehicle: { plate: plate.trim() || undefined, vin: vin.trim() || undefined, model: model.trim() || undefined, notes: notes.trim() || undefined },
        technicianId: selectedTechId,
      ...
      trackUxEvent({ name: "appointment_create", role: activeRole, ok: true, durationMs: Date.now() - formStartedAt });
  ...
  async function saveDraft() {
    ...
    try {
      const res = await createAppointmentDraft({
        client: { name: name.trim() || undefined, phone: phone.trim() || undefined, email: email.trim() || undefined, company: company.trim() || undefined },
        vehicle: { plate: plate.trim() || undefined, vin: vin.trim() || undefined, model: model.trim() || undefined, notes: notes.trim() || undefined },
  ...
  function stopPlateScan() {
    if (plateScanTimerRef.current) { window.clearInterval(plateScanTimerRef.current); plateScanTimerRef.current = null; }
    if (plateStreamRef.current) { plateStreamRef.current.getTracks().forEach((track) => track.stop()); plateStreamRef.current = null; }
  }
  ...
  async function startPlateScan() {
    ...
    try {
      ...
      if (plateVideoRef.current) { plateVideoRef.current.srcObject = stream; await plateVideoRef.current.play(); }
    } catch (e) {
  ...
  return (
    <main className="min-h-screen app-bg module-office px-4 mobile-nav-safe pt-4">
      {!canCreate ? (
        ...
        <>
          {/* Header */}
          <div className="mx-auto mt-4 flex w-full max-w-5xl items-center justify-between">
            <h1 className="text-lg font-extrabold text-slate-900">Nueva cita</h1>
            <button
              type="button"
              onClick={() => router.push("/calendario")}
              title="Cancelar y cerrar"
              className="rounded-2xl border-2 border-slate-200 bg-white p-2 text-slate-500 hover:border-red-300 hover:text-red-500"
            >
          ...
          <div className="mx-auto mt-4 w-full max-w-5xl space-y-4">
            ...
            {/* Sección 1: Cliente */}
            <section className="surface-content p-4">
              ...
              <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <input className="rounded-2xl border-2 border-slate-200 p-4 text-base font-semibold" placeholder="Teléfono*" value={phone} onChange={(e) => setPhone(e.target.value)} />
                ...
                <input className="rounded-2xl border-2 border-slate-200 p-4 text-base font-semibold sm:col-span-2" placeholder="Empresa" value={company} onChange={(e) => setCompany(e.target.value)} />
              </div>
            </section>
            ...
            <section className="surface-content p-4">
              ...
              <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <textarea className="rounded-2xl border-2 border-slate-200 p-4 text-base font-semibold sm:col-span-2" placeholder="Notas del vehículo" rows={3} value={notes} onChange={(e) => setNotes(e.target.value)} />
              </div>
            </section>

            {/* Sección 3: Trabajo y fecha */}
            <section className="surface-content p-4">
              <SectionTitle>Trabajo y fecha</SectionTitle>
              <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <section className="surface-content p-4">
              <div className="flex items-center gap-2">
                <SectionTitle>Técnico disponible</SectionTitle>
            ...
            {/* Sección 5: Hueco horario */}
            <section className="surface-content p-4">
              <div className="flex items-center gap-2">
                <SectionTitle>Hora libre</SectionTitle>
                {selectedTechId && techMap.get(selectedTechId) ? (
                  <span className="text-sm font-semibold text-slate-500">{techMap.get(selectedTechId)?.name}</span>
                ) : null}
            ...
            {/* Mensajes y acciones */}
            {error ? <p className="rounded-2xl border-2 border-red-200 bg-red-50 p-3 text-sm font-semibold text-red-700">{error}</p> : null}
            {okMsg ? <p className="rounded-2xl border-2 border-emerald-200 bg-emerald-50 p-3 text-sm font-semibold text-emerald-700">{okMsg}</p> : null}
            {lastWhatsappUrl ? (
            ...
            <div className="flex flex-col gap-2 pb-6 sm:flex-row sm:justify-end">
              <button onClick={() => void saveDraft()} disabled={!canDraft || saving} className="rounded-2xl border-2 border-slate-300 bg-white px-6 py-3 text-sm font-extrabold text-slate-800 disabled:opacity-40">
                Guardar borrador
              </button>
              <button onClick={() => void saveAppointment()} disabled={!canSave || saving} className="cta-primary px-6 py-3 text-sm disabled:opacity-40">
          ...
          <datalist id="client-name-suggestions">
            {quickHistory.map((row) => <option key={`n_${row.phone}_${row.name}`} value={row.name} />)}
          </datalist>
          <datalist id="client-phone-suggestions">
            {quickHistory.map((row) => <option key={`p_${row.phone}`} value={row.phone} />)}
          </datalist>
          <datalist id="plate-suggestions">
            {quickHistory.filter((row) => row.plate).map((row) => <option key={`m_${row.plate}`} value={row.plate} />)}
          </datalist>
